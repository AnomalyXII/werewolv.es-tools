...
public class ArchivedGameParserTest {
    ...
    public void parse_should_correctly_parse_an_archived_game() throws Exception {
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("archive/ext-090.json")) {
            ...
            assertEquals(game.getDays().size(), 4);
            // Day 1 and Night 1
            assertEquals(game.getDay(0).getDayPhase().getEvents().size(), 18);
            ...
            // Day 1 and Night 2
            assertEquals(game.getDay(1).getDayPhase().getEvents().size(), 320);
            ...
            // Day 1 and Night 3
            assertEquals(game.getDay(2).getDayPhase().getEvents().size(), 341);
            ...
            // Day 1 and Night 4
            assertEquals(game.getDay(3).getDayPhase().getEvents().size(), 280);
